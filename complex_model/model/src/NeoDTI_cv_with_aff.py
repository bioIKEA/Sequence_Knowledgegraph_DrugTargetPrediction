...
class Model(nn.Module):
    def __init__(self):
        ...
        self.dpar = bi_layer(dim_drug,dim_drug, sym=False, dim_pred=dim_pred)
        self.dpr = bi_layer(dim_drug,dim_drug, sym=False, dim_pred=dim_pred)
        
    def forward(self, drug_drug, drug_drug_normalize, drug_drug_mask,
                drug_chemical, drug_chemical_normalize,
                drug_disease, drug_disease_normalize, drug_disease_mask,
                drug_sideeffect, drug_sideeffect_normalize, drug_sideeffect_mask,
                ...
                drug_protein_mask,
                drug_protein_affinity, drug_protein_affinity_normalize,
                protein_drug_affinity, protein_drug_affinity_normalize,
                drug_protein_affinity_mask
                ):
        ...
        drug_vector1 = torch.nn.functional.normalize(F.relu(torch.matmul(
                ...
                torch.matmul(drug_protein_normalize, a_cul(self.protein_embedding, *self.dpn_pe_w)) + \
                torch.matmul(drug_protein_affinity_normalize, a_cul(self.protein_embedding, *self.dpan_pe_w)) + drug_encoder_data, \
        ...
        protein_vector1 = torch.nn.functional.normalize(F.relu(torch.matmul(
                ...
                torch.matmul(protein_drug_normalize, a_cul(self.drug_embedding, *self.pv_pdn_de_w)) + \
                torch.matmul(protein_drug_affinity_normalize, a_cul(self.drug_embedding, *self.pv_pdan_de_w)) + protein_encoder_data, \
        ...
        tmp_drug_drug = torch.multiply(drug_drug_mask, (drug_drug_reconstruct-drug_drug))
        drug_drug_reconstruct_loss = torch.sum(torch.multiply(tmp_drug_drug, tmp_drug_drug))
        ...
        tmp_drug_disease = torch.multiply(drug_disease_mask, (drug_disease_reconstruct-drug_disease))
        drug_disease_reconstruct_loss = torch.sum(torch.multiply(tmp_drug_disease, tmp_drug_disease))

        drug_sideeffect_reconstruct = bi_cul(drug_representation,sideeffect_representation, *self.dsr)
        tmp_drug_sideeffect = torch.multiply(drug_sideeffect_mask, (drug_sideeffect_reconstruct-drug_sideeffect))
        drug_sideeffect_reconstruct_loss = torch.sum(torch.multiply(tmp_drug_sideeffect, tmp_drug_sideeffect))
        ...
        drug_protein_affinity_reconstruct = bi_cul(drug_representation,protein_representation, *self.dpar)
        tmp_drug_protein_affinity = torch.multiply(drug_protein_affinity_mask, (drug_protein_affinity_reconstruct-drug_protein_affinity))
        drug_protein_affinity_reconstruct_loss = torch.sum(torch.multiply(tmp_drug_protein_affinity, tmp_drug_protein_affinity)) #/ (torch.sum(self.drug_protein_mask)
        ...
        loss = drug_protein_reconstruct_loss + 1.0*(drug_drug_reconstruct_loss+drug_chemical_reconstruct_loss+
                                                            ...
                                                            protein_disease_reconstruct_loss+drug_protein_affinity_reconstruct_loss) + l2_loss
...
def train_and_evaluate(DTItrain, DTIvalid, DTItest, aff, round, verbose=True, num_steps = 4000):
    ...
    for i in range(num_steps):
        ...
        model.train()
        mask_affinity_ = cuda(torch.tensor(mask_affinity))
        protein_drug_affinity_normalize_ = cuda(torch.tensor(protein_drug_affinity_normalize))
        protein_drug_affinity_ = cuda(torch.tensor(protein_drug_affinity))
        drug_protein_affinity_normalize_ = cuda(torch.tensor(drug_protein_affinity_normalize))
        drug_protein_affinity_ = cuda(torch.tensor(drug_protein_affinity))
        ...
        global drug_sideeffect_mask
        drug_sideeffect_mask_ = cuda(torch.tensor(drug_sideeffect_mask))
        ...
        global drug_disease_mask
        drug_disease_mask_ = cuda(torch.tensor(drug_disease_mask))
        ...
        global drug_drug_mask
        drug_drug_mask_ = cuda(torch.tensor(drug_drug_mask))
        ...
        tloss, dtiloss, results = model(
                                    drug_drug_, drug_drug_normalize_, drug_drug_mask_,\
                                    drug_chemical_, drug_chemical_normalize_,\
                                    drug_disease_, drug_disease_normalize_, drug_disease_mask_,\
                                    drug_sideeffect_, drug_sideeffect_normalize_, drug_sideeffect_mask_,\
                                    ...
                                    mask_,\
                                    drug_protein_affinity_, drug_protein_affinity_normalize_,\
                                    protein_drug_affinity_, protein_drug_affinity_normalize_,\
                                    mask_affinity_)
        ...
        if i % 25 == 0 and verbose == True:
            ...
            if valid_aupr >= best_valid_aupr:
                torch.save(model, 'model_aff_round_%s.pkl' % (round))
...
for r in range(10):
    ...
    for train_index, test_index in kf:
        ...
        # print(DTItrain.shape) # 18085x3
        # print(DTItest.shape)  # 2116x3
        # print(DTIvalid.shape) # 952x3
        v_auc, v_aupr, t_auc, t_aupr = train_and_evaluate(DTItrain=DTItrain, DTIvalid=DTIvalid, DTItest=DTItest, aff=affinity_network, 
            round = str(r) + 'cv_' + str(cv), num_steps=30)
    ...
    np.savetxt('test_auc_aff_%s' % (r), test_auc_round)
    np.savetxt('test_aupr_aff_%s'% (r), test_aupr_round)
